...
export const profileApi = {
  updateProfile: () => {
    return useMutation({
      mutationFn: async (profile: Partial<ProfileFormData>): Promise<ProfileFormData> => {
        const response = await axiosInstance.patch(`${USER_SERVICE_API_URL}/`, profile);
  ...
  setOrUpdateAvatar: () => {
    return useMutation({
      mutationFn: async (file: File): Promise<RefreshResponseBody> => {
        ...
        const response = await axiosInstance.put(`${USER_SERVICE_API_URL}/avatar`, formData, {
          headers: {
  ...
  deleteAvatar: () => {
    return useMutation({
      mutationFn: async () => {
        const response = await axiosInstance.delete(`${USER_SERVICE_API_URL}/avatar`)
        return response.data;
      },
    });
  },
